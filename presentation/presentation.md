...
* **Implementation :** `source/Model.jl` using the **Builder Pattern** (`AsrModelBuilder`) and HiGHS solver. **Parallel execution infrastructure** via `scripts/t0_execution.sh` / `scripts/t1_execution.sh` launching concurrent instances on server cores.
...
| `t0_execution.sh` / `t1_execution.sh` | instantiate env, launch 10 instances in parallel (bash `&`) |
| `run_on_server.sh` | detached launcher (`nohup` / `tmux`) |
...
* Fluent builder-pattern model + HiGHS; lexicographic descent (OWA gadget) + warm start; headless parallel runner (bash); result persistence (`Result.jl`); plotting + presentation.