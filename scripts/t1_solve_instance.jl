...
function parse_args(args)
    # Default like the notebooks: a fresh t1_results/<yyyymmdd-HHMMSS> run dir
    # (get_runDir_by_timestamp), so a bare run lands in its own timestamped folder
    # rather than the flat t1_results/. Wrappers still override this via --output.
    resultsDir   = get_runDir_by_timestamp(joinpath(REPO_ROOT, "t1_results"))
